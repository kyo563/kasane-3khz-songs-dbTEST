...
1. **`archive` は小さめ `limit` をページ分割して取得する**
   - 1回の大量レスポンスを避けるため、`limit=30` 前後で `offset` を進めながら複数回取得する。
   - 取得済みキーの重複を除去し、同じページが返ってきた場合は早期打ち切りする。
...
2. **上限とオフセットを変えて増分確認**
   - `limit=10`, `30`, `50` を順に実行し、必要なら `offset=30`, `60` を付けて `rows.length` が増えるかを見る。
3. **キーワード確認（実データ存在確認）**
